...
angular
  ...
  .controller("ResourceCtrl", function(
    ...
    $scope.CurrentFile ={};
    $scope.Folder = { PreviousPath: "" ,PathHistory:[]};
    $scope.goToFolder = function(item) {
      Dropbox.listFolder(item.id, {
        Authorization:
          "Bearer " + $rootScope.GlobalSettings.DropBox.Access_Token_Global
      }).then(function(data) {
        $scope.Folder.PathHistory.push({name:item.name,id:item.id,content:data.entries})
    ...
    $scope.goBack = function(){
      $scope.Folder.PathHistory.pop();
      $scope.Folder.content = $scope.Folder.PathHistory[$scope.Folder.PathHistory.length-1].content;
     
    }
    $scope.goToHomeFolder = function(){
      $scope.Folder.PathHistory = [];
      $scope.goToFolder({ id: "" ,name:"Home"});
    }
    $scope.goToBreadCrumb = function(index){
      $scope.Folder.PathHistory = $scope.Folder.PathHistory.slice(0,index);
      $scope.Folder.content = $scope.Folder.PathHistory[$scope.Folder.PathHistory.length-1].content;
    }
